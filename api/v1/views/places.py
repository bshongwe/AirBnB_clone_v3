#!/usr/bin/python3
"""
Create Place objects view - handles all default RESTful API actions
"""
...
def places_search():
    ...
    if 'states' in data:
        states_obj = [storage.get(State, s_id) for s_id in data['states']]
    ...
    if 'cities' in data:
        city_obj = [storage.get(City, c_id) for c_id in data['cities']]
    ...
    if 'amenities' in data:
        amenities_obj = [storage.get(Amenity, a_id) for a_id in data['amenities']]
        for amenity in amenities_obj:
            filtered_places = [place for place in filtered_places if amenity in place.amenities]
